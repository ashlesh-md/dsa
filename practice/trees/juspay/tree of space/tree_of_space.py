...
class LockableTree:
    ...
    def lock(self, node_id, user_id):
        ...
        if not node or self.is_locked_ancestor(node) or self.is_locked_descendant(node):
            return False
    ...
    def upgrade(self, node_id, user_id):
        ...
        if (
            not node
            or node.locked
            or not self.are_descendants_locked_by_same_user(node, user_id)
        ):
            return False
    ...
    def are_descendants_locked_by_same_user(self, node, user_id):
        return all(
            child.locked_by == user_id
            or not self.are_descendants_locked_by_same_user(child, user_id)
...
print(tree.unlock(4, "user2"))  # False
print(tree.lock(4, "user1"))  # False
